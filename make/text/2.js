...
function makeTask(bind, task) {
  ...
  task.zone.forEach(zone => {
    switch (zone.form) {
      case `host`:
        makeHost(zone, bind).forEach(line => {
          t.push(`  ${line}`)
        })
        break
      case `save`:
...
function makeHost(zone, bind) {
  const left = getName(bind, zone.name)
...
function makeNest(nest, loadState) {
  const path = nest.stem.map((stem, i) => {
    switch (stem.form) {
      case 'term':
        if (i === 0) {
          return loadState ? getName(loadState, [stem.name]) : stem.name
        } else {
          return `['${stem.name}']`
        }
        break
...
function makeDockCallCallBase(call, loadState) {
  ...
  if (object.sift.form === 'link') {
    ...
    const obj = getName(loadState, object.sift.text)
