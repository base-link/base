export function load_codeCard_call(load) {
    ...
    code.assumeNest(load).forEach((nest, index) => {
        code.addTask(load.base, () => {
            load_codeCard_call_nestedChildren(code.withLink(colorInput, nest, index));
        });
    });
}
export function load_codeCard_call_nestedChildren(load) {
    ...
    switch (type) {
        case LinkHint.DynamicTerm: {
            ...
            if (index === 0) {
                const term = code.createBlueTerm(code.assumeLink(load, Link.Term));
        ...
        case LinkHint.DynamicPath: {
            const index = code.assumeLinkIndex(load);
            if (index === 0) {
                const path = code.createBluePath(code.assumeLink(load, Link.Path));
