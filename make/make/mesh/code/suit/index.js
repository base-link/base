export function load_codeCard_suit(load) {
    const red = code.pushRed(load, code.createRedGather(load, 'classInterface'));
    ...
    code.assumeNest(colorInput).forEach((nest, index) => {
        code.addTask(colorInput.base, () => {
            code.load_codeCard_suit_nestedChildren(code.withLink(colorInput, nest, index));
        });
    });
}
export function load_codeCard_suit_nestedChildren(load) {
