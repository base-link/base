...
export function load_codeCard_load(load) {
    ...
    card.assumeNest(colorInput).forEach((nest, index) => {
        load_codeCard_load_nestedChildren(card.withLink(colorInput, nest, index));
    });
}
export function load_codeCard_load_nestedChildren(load) {
    ...
    switch (type) {
        case LinkHint.StaticText: {
            ...
            if (index !== 0) {
                ...
                card.pushRed(load, card.createRedValue(load, 'absolutePath', bluePath));
