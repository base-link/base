...
export function load_codeCard_head(load) {
    const red = card.pushRed(load, card.createRedGather(load, 'typeInputs'));
    ...
    card.assumeNest(colorInput).forEach((nest, index) => {
        tool.loadTask(load.base, () => {
            card.load_codeCard_head_nestedChildren(card.withLink(load, nest, index));
        });
    });
}
export function load_codeCard_head_nestedChildren(load) {
