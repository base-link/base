...
export function generateChangeVariableTypeError(input) {
    return {
        code: '0031',
...
export function generateHighlightedErrorForLinkTree(input) {
    const highlightedRange = code.getCursorRangeForTree(input);
    return code.generateHighlightedError(input.module.textByLine, highlightedRange);
}
export function generateHighlightedErrorForText(input) {
...
export function generateInvalidDeckLink(input, link) {
    return {
        code: `0008`,
        note: `Invalid deck link '${link}'.`,
    };
}
export function generateInvalidNestCaseError(input, type) {
    let scope;
    try {
...
export function generateInvalidNestChildrenLengthError(input, length) {
    return {
        code: `0009`,
...
//   }
// }
export function generateInvalidWhitespaceError(input) {
    const token = input.tokenList[input.state.index];
    code.assertTextGenericType(token);
    const highlightedRange = code.getCursorRangeForTextWhitespaceToken(token, input);
    const text = code.generateHighlightedError(input.textByLine, highlightedRange);
    return {
        code: '0027',
        file: input.path,
...
export function generateModuleUnresolvableError(input) {
    return {
        code: '0020',
        file: `${input.module.path}`,
...
export function generateStringMismatchError(input, a, b) {
    return {
        code: '0030',
        file: input.path,
...
export function generateSyntaxTokenError(input, lastToken) {
    const highlight = {
        end: {
            character: 0,
            line: 0,
    ...
    const text = code.generateHighlightedError(input.textByLine, highlight);
    return {
        code: `0021`,
...
export function generateTermMissingError(input, type, object) {
    const { module } = input;
    return {
        code: `0018`,
        file: `${module.path}`,
...
export function generateUnhandledNestCaseBaseError(input) {
    const { module } = input;
    const text = code.generateHighlightedErrorForLinkTree(input);
    return {
        code: `0005`,
...
export function generateUnhandledNestCaseError(input, type) {
    let scope;
    try {
...
export function generateUnhandledTermCaseError(input) {
    let scope;
    try {
        // scope = code.resolveTermString(input, 1)
        scope = code.resolveTermString(input);
...
export function generateUnhandledTermInterpolationError(input) {
    return {
        code: `0001`,
        file: `${input.module.path}`,
...
export function generateUnknownTermError(input) {
    const { module } = input;
    const name = code.resolveTermString(input);
    const text = code.generateHighlightedErrorForLinkTree(input);
...
export function generateUnresolvedPathError(input, path) {
    const { module } = input;
    return {
        code: `0013`,
        file: module.path,
...
export function getCursorRangeForPath(input) {
    ...
    const start = getCursorRangeForTerm(code.withLink(input, firstSegment));
...
export function getCursorRangeForPlugin(input) {
...
export function getCursorRangeForString(input) {
...
export function getCursorRangeForTerm(input) {
...
export function getCursorRangeForText(input) {
    ...
    if (first.type === Link.String) {
        firstRange = code.getCursorRangeForString(code.withLink(input, first));
    }
    else if (first.type === Link.Plugin) {
        firstRange = code.getCursorRangeForPlugin(code.withLink(input, first));
    }
    else {
        code.throwError(code.generateInvalidCompilerStateError());
        throw new CompilerError();
...
export function getCursorRangeForTextWhitespaceToken(token, input) {
    let tokens = [];
...
export function getCursorRangeForTree(input) {
    // console.log(input)
