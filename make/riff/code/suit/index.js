...
export function process_codeCard_suit(input) {
    const red = code.pushRed(input, code.createRedGather(input, 'classInterface'));
    ...
    code.assumeNest(colorInput).forEach((nest, index) => {
        code.addTask(colorInput.base, () => {
            code.process_codeCard_suit_nestedChildren(code.withLink(colorInput, nest, index));
        });
    });
}
export function process_codeCard_suit_nestedChildren(input) {
