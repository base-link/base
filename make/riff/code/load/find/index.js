...
export function process_codeCard_load_find(input) {
    ...
    code.assumeNest(colorInput).forEach((nest, index) => {
        code.addTask(input.base, () => {
            code.process_codeCard_load_find_nestedChildren(code.withLink(colorInput, nest, index));
        });
    });
}
export function process_codeCard_load_find_nestedChildren(input) {
...
export function process_codeCard_load_find_staticTerm(input) {
...
export function process_find_scope(input) {
    ...
    code.pushRed(input, code.createRedGather(input, 'scope', [scopeString]));
