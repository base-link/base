export function load_deckCard_deck_bear(load) {
    code.loadLink(load).forEach((nest, index) => {
        code.addTask(load.base, () => {
            code.load_deckCard_deck_bear_nestedChildren(code.withLink(load, nest, index));
        });
    });
}
    ...
    if (index === 0) {
        ...
        switch (type) {
            case LinkHint.StaticText: {
                ...
                code.pushRed(load, code.createRedValue(load, 'bear', blueString));
            ...
            case LinkHint.DynamicText: {
                ...
                code.attachBlue(load, 'bear', code.createBlueText(load, textNode));
                code.bindText(load, () => {
                    const string = code.assumeText(load);
                    const path = code.resolveModulePath(load, string);
