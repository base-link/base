export function load_codeCard_load_find(load) {
    ...
    code.assumeNest(colorInput).forEach((nest, index) => {
        code.addTask(load.base, () => {
            code.load_codeCard_load_find_nestedChildren(code.withLink(colorInput, nest, index));
        });
    });
}
export function load_codeCard_load_find_nestedChildren(load) {
...
export function load_codeCard_load_find_staticTerm(load) {
...
export function load_find_scope(load) {
