...
export function process_codeCard_load_take_bear(
  scope: ScopeType<Scope.Nest>,
): void {
  scope.data.nest.nest.forEach((nest, index) => {
    const nestedScope = api.extendNest(scope, nest, index)
    api.process_codeCard_load_take_bear_nestedChildren(
      nestedScope,
    )
  })
}

export function process_codeCard_load_take_bear_nestedChildren(
  scope: ScopeType<Scope.Nest>,
): void {
